...
// title, headline summary, an asciigraph trend line, a streak indicator, a
// usage breakdown, and a closing attribution line, in that order.
package render
...
// repoURL is token-profile's own repo, credited on the card's last content
// line so anyone who sees the rendered card in a README can find the tool
// that produced it.
...
func Render(ds snapshot.MergedDataset, sum summary.Summary, mode config.BreakdownMode, breakdownLimit int, renderedAt time.Time) string {
	...
	lines = append(lines, generatedByLine())
	return box(lines)
}
