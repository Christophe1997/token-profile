...
// TestRender_GeneratedByLineIsLastContentLine covers attribution: the card's
// very last content line, immediately above the bottom border, credits
// token-profile as a markdown link back to its repo — so anyone who sees
// the rendered card in a README can find the tool that produced it.
func TestRender_GeneratedByLineIsLastContentLine(t *testing.T) {
	ds := fixtureDataset()
	asOf := time.Date(2026, 6, 22, 12, 0, 0, 0, time.UTC)
	sum := summary.Compute(ds, asOf, 30*24*time.Hour)
	renderedAt := time.Date(2026, 6, 22, 14, 0, 0, 0, time.UTC)
	...
	last := lines[len(lines)-2] // lines[len(lines)-1] is the bottom border
	if !strings.Contains(last, want) {
		t.Errorf("Render() last content line = %q, want it to contain %q", last, want)
	}
}
